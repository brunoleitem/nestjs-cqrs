...
export class TwilioClient {
    ...
    async getAccount(accountName: string) {
        return await this.client.api.v2010.accounts.list({ friendlyName: accountName })
    }

    async createSubAccount(accountName: string) {
        return await this.client.api.v2010.accounts.create({ friendlyName: accountName })
    }

    /* Enviar com body causa um erro 63016
    Em caso de whatsapp deve enviar com template.
    */
    async sendMessage(body: string, from: string, to: string, sid: string) {
    ...
    async createTemplate(type: Types, friendlyName: string, variables: { [key: string]: string }) {
